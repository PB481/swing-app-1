...
DATE_COLUMNS = {
    "client_roster": ["last_reprice_date"],
    "active_rfps": ["due_date"],
    "change_pipeline": ["blocked_since"],
    "incidents_log": ["event_date"],
    "action_items": ["due_date", "created_date"],
    "meeting_notes": ["meeting_date"],
    "stakeholders": ["last_interaction_date"],
}


def load_table(table_name: str) -> pd.DataFrame:
    """Load full table into DataFrame. Called fresh after every write."""
    df = run_query_df(f"SELECT * FROM {table_name}")
...
# Required user-supplied NOT NULL columns per table. New rows with any of
# these blank are dropped silently before write (the SQLite AUTOINCREMENT id
# fills itself in, so users only need to supply these).
REQUIRED_COLUMNS = {
    ...
    "action_items": ["task"],
    "meeting_notes": ["meeting_date", "title"],
    "stakeholders": ["name"],
}


def _to_sql_value(val):
    """Convert a DataFrame cell into a SQLite-friendly scalar."""
    if pd.isna(val):
        return None
...
def _drop_incomplete_rows(table_name: str, df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    ...
    for col in present:
        col_mask = df[col].notna() & df[col].astype(str).str.strip().ne("")
        mask &= col_mask
...
def write_table(table_name: str, df: pd.DataFrame):
    ...
    """
    df, dropped = _drop_incomplete_rows(table_name, df)
    if dropped:
        st.warning(
            f"Skipped {dropped} row(s) with missing required field(s): "
            f"{', '.join(REQUIRED_COLUMNS.get(table_name, []))}."
        )
    ...
    try:
        ...
        if not df.empty:
            ...
            if cols:
                ...
                for _, row in df.iterrows():
                    vals = tuple(
                        _to_sql_value(row[c]) for c in cols
                    )
                    cur.execute(
